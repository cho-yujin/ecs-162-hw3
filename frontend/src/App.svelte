<script lang="ts">
  ...
  onMount(async () => {
    ...
    // Construct url
    const url =
      "https://api.nytimes.com/svc/search/v2/articlesearch.json?q=Sacramento&api-key=" +
      apiKey; // added the apiKey that you recieve from the env + backend here
    console.log("URL:", url);
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (firstTitle) {
        // ts is being a mean so i needed to check that it wasn't null
