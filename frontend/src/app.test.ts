...
test("Articles fetched are related to Sacramento or Davis", async () => {
  for (const dataObject of data.response.docs) {
    ...
    keywords
      ...
      .filter(
        (keyword: any) =>
          keyword.value.includes("sacramento") ||
          keyword.value.includes("davis")
      );
...
test("Date at top of the page is correct", () => {
  ...
  let correctDate = screen
    .getByTestId("dateInfo")
    .textContent!.includes(todaysdate);
  // screen.debug(screen.getByTestId('dateInfo'));
  assert(correctDate);
});

// HW3 tests begin here!!

test("Comments filter by articleID properly", () => {
  const testComments = [
    {
      _id: "id",
      article_id: "Correct article",
      username: "Milo",
      comment: "Mewo",
    },
    {
      _id: "id",
      article_id: "Incorrect article",
      username: "Milo",
      comment: "I am quite smart",
    },
    {
      _id: "id",
      article_id: "Correct article",
    ...
    {
      _id: "id",
      article_id: "Devious article",
      username: "Yujin",
      comment: "I am probably milo",
    },
  ];
  const correct_article_id = "Correct article";

  const newComments = filterComments(testComments, correct_article_id);
  const isFilteredCorrectly = newComments.every(
    (comment: any) => comment.article_id === correct_article_id
  );

  assert(isFilteredCorrectly);
});
